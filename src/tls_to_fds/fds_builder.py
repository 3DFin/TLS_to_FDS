...
def generate_fuel_block(
    ...
    ember_density = safe_get(
        env_params, "ember_density", get_default("env_params", "ember_density", 62.5)
    )
    ember_velocity = safe_get(
        env_params, "ember_velocity", get_default("env_params", "ember_velocity", 0.0)
    )
...
def generate_bfm_surf(ground_fuels: Any, active_preset: Dict[str, Any]) -> str:
    ...
    surf_str = "&SURF ID = 'Synthetic Ground Fuel',\n"
...
def assemble_fds_file(
    ...
    ign_dur = safe_get(
        env_params, "ign_duration", get_default("env_params", "ign_duration", 30.0)
    )
    ign_pattern = safe_get(
        env_params,
        "ign_pattern",
        get_default("env_params", "ign_pattern", "Line: South Edge (y_min)"),
    )
    vent_width = safe_get(
        env_params, "vent_width", get_default("env_params", "vent_width", 1.0)
    )

    wind_speed = safe_get(
        env_params, "wind_speed", get_default("env_params", "wind_speed", 3.0)
    )
    wind_dir = safe_get(
        env_params, "wind_dir", get_default("env_params", "wind_dir", 15.0)
    )
    obukhov = safe_get(
        env_params, "obukhov", get_default("env_params", "obukhov", -350.0)
    )
    ...
    with open(fds_path, "w") as file:
        file.write(
            f"&HEAD CHID='{sim_name}', TITLE='TLS_to_FDS Generated Simulation'\n"
        )
        file.write(f"&TIME T_END={total_time} /\n\n")

        file.write(
            generate_mesh_block(
                base_bounds, sky_bounds, nx, ny, nz, domain_params, base_voxel
            )
        )

        file.write("!! ENVIRONMENT & IGNITION\n")
        file.write(
            f"&WIND SPEED={wind_speed:.2f}, DIRECTION={wind_dir:.2f}, L={obukhov:.2f}, Z_0={z0:.2f} /\n\n"
        )

        file.write(
            f"&SURF ID='IGN FIRE', HRRPUA={hrrpua:.2f}, COLOR='RED', RAMP_Q='fireramp' /\n"
        )
        ...
        file.write(
            f"&RAMP ID='fireramp', T={wind_dev + ign_dur + 1.0:.2f}, F=0.0 /\n \n"
        )

        file.write(
            f"&VENT XB={ign_x_min:.2f},{ign_x_max:.2f},{ign_y_min:.2f},{ign_y_max:.2f},{z_min:.2f},{z_min:.2f}, "
